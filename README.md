...
| # | 功能 | 回答什么 | 靠什么 | 产出 |
| --- | --- | --- | --- | --- |
| 1 | 材料解析 | 客户给了什么 | TextIn 解析出带页码的原文；系统按类型归档，并摘出项目要点（只记事实） | 项目要点（每条带来源页码）· 材料里的需求（事实留档）· 材料清单 · 解析状态 |
| 2 | 需求确认 | 客户到底要什么、这条我们能不能做 | DeepSeek 抽取结构化需求并回填来源页码；人点「确认」= 进基线，并立刻对着企业资料做能力判断 | 需求清单（草稿/基线）· 每条需求的能力结论（完全支持 / 部分支持 / 暂不支持 / 待补依据）· 风险与依据 |
| 3 | 售前建议 | 还剩什么没定、下一步怎么办 | 检索企业能力库与历史案例，规则层先给结论上限，DeepSeek 再组织表达 | 整体结论与匹配度 · 风险提示 · 要问客户/要问内部/要同步销售 · 不可承诺清单 |

三个功能的边界：材料解析只讲事实（材料里写着什么，含还没确认的需求草稿）；需求确认讲这条需求算不算数、我们能不能做；售前建议讲剩下的问题与下一步（风险、要问谁）。项目要点只做核对、编辑、忽略，不进入确认流程。

需求确认页「一条需求一张卡」：点「确认」= 进基线 + 立刻判断，结论、风险与依据都长在这一张卡上（依据可就地展开，含人工覆写与留痕），不用切页找；结论条上的「能力判断」报一份分档分布（待补依据 → 暂不支持 → 部分支持 → 完全支持）。
售前建议页只回答「还剩什么没定」：一句结论 + 两句概括（风险 / 下一步）常驻，两句都能点；
