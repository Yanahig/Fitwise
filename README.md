...
| 3 | 售前建议 | 还剩什么没定、下一步怎么办 | 汇总判断的后果与所有待澄清问题，并把它们分成三份清单；执行清单由 DeepSeek 排 | 整体结论与匹配度 · 风险提示 · 要问客户/要问内部/要同步销售 · 下一步行动 · 不可承诺清单 |
...
下面是三块：风险提示按高 → 中 → 低分区（每条都能点回它依据的那条需求）、「要问谁」（要问客户 / 要问内部 / 要同步销售，标出哪条影响判断、哪条只影响承诺）、「下一步行动」（先办哪一件、谁办、什么时候，按优先级与截止时间排）。
「要同步销售」是给销售 / 商务的**信息**（承诺边界、报价与周期口径），不是待办；「下一步行动」是执行清单 —— 两者原本混在一栏里，第三十九轮拆开了。

「项目管理」列全部项目，每行「打开」右侧有「删除」：确认框会报出连带删掉的条数（材料 / 需求 / 能力结论），删完侧栏的「我的项目」同步刷新，磁盘上的材料文件一并清掉。

「下一步行动」每行右侧有「已完成」：点了这条置灰、沉到列表最后，结论条上的「下一步」自动落到还没做完的第一条；再点一次可撤销。完成状态只记在本机（`fitwise.done-actions.<项目 id>`），不落库、不做派工 —— 跟踪与审批仍属于 P1。
